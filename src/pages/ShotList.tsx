...
const COLUMNS = [
  { key: 'shotNumber',           label: '#',          width: 56 },
  { key: 'scene',                label: 'SCENE',       width: 92 },
  { key: 'intExt',               label: 'INT/EXT',     width: 80 },
  { key: 'location',             label: 'LOCATION',    width: 150 },
  { key: 'shotType',             label: 'SHOT',        width: 80 },
  { key: 'movement',             label: 'MOVEMENT',    width: 104 },
  { key: 'lens',                 label: 'LENS',        width: 70 },
  { key: 'description',          label: 'DESCRIPTION', flex: true },
  { key: 'cast',                 label: 'CAST',        width: 115 },
  { key: 'specialEquipment',     label: 'EQUIP',       width: 104 },
  { key: 'estimatedSetupMinutes',label: 'SETUP',       width: 70 },
  { key: 'actions',              label: '',            width: 70 },
]

const cellStyle: React.CSSProperties = {
  fontFamily: '"DM Mono", monospace',
  /* This table IS the shot list — it gets a data size, not a caption size.
     Column widths above are sized for it. */
  ...
  return (
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      ...
      >
        ...
        >
          <IconPlus size={12} />
          <span className="font-mono" style={{ fontSize: 'var(--text-2xs)', letterSpacing: '0.06em' }}>ADD SHOT</span>
        </button>
      </div>
      ...
      <div style={{ flex: 1, overflowX: 'auto', overflowY: 'auto' }}>
        {projectShots.length === 0 ? (
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse', minWidth: 1000 }}>
            {/* Column headers */}
            <thead>
              <tr style={{ borderBottom: '1px solid var(--color-border-subtle)', background: 'var(--color-surface)' }}>
                {COLUMNS.map(col => (
                  <th
                    key={col.key}
            ...
            <tbody>
              {filtered.map(shot => {
                ...
                return (
                  ...
                  >
                    {/* Shot # */}
                    <td style={{ ...cellStyle, width: 56, padding: '8px 8px' }}>
                      <span style={{
                        display: 'inline-flex',
                        alignItems: 'center',
                        gap: 4,
                    ...
                    {/* Scene */}
                    <td style={{ ...cellStyle, width: 92 }}>
                      {sceneInfo ? (
                        <span style={{ color: 'var(--color-text-tertiary)' }}>
                          SC {String(sceneInfo.index + 1).padStart(2, '0')}
                        </span>
                      ) : (
                        <span style={{ color: 'var(--color-danger)', fontSize: 'var(--text-2xs)' }}>—</span>
                      )}
                    </td>

                    {/* INT/EXT */}
                    <td style={{ ...cellStyle, width: 80, color: 'var(--color-text-tertiary)' }}>{shot.intExt}</td>

                    {/* Location */}
                    <td style={{ ...cellStyle, width: 150 }}>{shot.location || '—'}</td>
                    ...
                    {/* Movement */}
                    <td style={{ ...cellStyle, width: 104, color: 'var(--color-text-tertiary)' }}>{shot.movement}</td>

                    {/* Lens */}
                    <td style={{ ...cellStyle, width: 70, color: 'var(--color-text-tertiary)' }}>{shot.lens}mm</td>
                    ...
                    {/* Cast */}
                    <td style={{ ...cellStyle, width: 115, color: 'var(--color-text-tertiary)' }}>
                      {shot.cast.length > 0 ? shot.cast.join(', ') : '—'}
                    </td>

                    {/* Equipment */}
                    <td style={{ ...cellStyle, width: 104, color: 'var(--color-text-tertiary)' }}>
                      {shot.specialEquipment || '—'}
                    </td>

                    {/* Setup */}
                    <td style={{ ...cellStyle, width: 70, color: 'var(--color-text-tertiary)' }}>
